      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-8 lg:gap-12 justify-items-center">
        {mediaLogos.map((media, index) => (
          <div key={index} className="flex">
            <div className={`w-24 h-24 md:w-28 md:h-28 rounded-full bg-white flex items-center justify-center ${index % 2 === 0 ? 'overflow-visible' : 'overflow-hidden'}`}>
              <div className={`relative w-full h-full ${index % 2 === 0 ? 'scale-125' : ''}`}>
                <Image
                  src={media.logo}
                  alt={media.name}
                  fill
